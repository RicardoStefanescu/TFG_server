...
# Fonts
font_file = "resources/SF-Pro-Display-Medium.otf"
font_bold = "resources/SF-Pro-Display-Bold.otf"
header_font_size = 32
...
def generate_verified_image(final_image, is_verified, twitter_name_width):	
	if is_verified:
		...
		generate_white_image('resources/verified.png', verified_image_white_file)
	...
	for line in textwrap.wrap(text, width=54):
		if '@' in line or '#' in line or contains_url(line):
			...
			for index, part in enumerate(string_parts):
				if part and ('@' == part[0] or '#' == part[0] or is_valid_url(part)):
					color = links_color
				else:
					color = 'white'
...
def generate_images_and_get_final_y(final_image, images, y_position):
	if images != '' and images != None:
		...
		tweet_image = Image.open(image_url)
...
def get_image_from_file(image_url):
	image_file = 'tweet-image.jpg'
	urllib.request.urlretrieve(image_url, image_file)
	tweet_image = Image.open(image_file, 'r')
	return tweet_image
...
def download_and_insert_image(final_image, image_path):
	tweet_image = Image.open(image_path)
	tweet_image = tweet_image.resize((96, 96), Image.ANTIALIAS)
...
def gen(twitter_name, twitter_account, text, image_url, date_text, is_verified, images=""):
