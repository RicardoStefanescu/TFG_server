...
## Added by R-st
def face_swap(swap_index, source_image, target_image, reconstruction_module, segmentation_module, face_parser=None,
    ...
    with torch.no_grad():
        ...
        if not cpu:
            source = source.cuda()
        target = torch.tensor(np.array([target_image])[np.newaxis].astype(np.float32)).permute(0, 4, 1, 2, 3)
